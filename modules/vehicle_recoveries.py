...
file_name = "data_files/vehicle_recoveries.csv"
...
def add(vehicle_no, vehicle_type):
    ...
    for row in rows :
        if row['vehicle_no'] == vehicle_no and row['vehicle_type'] == vehicle_type :
            response_msg = {'status': 'error', 'msg': "recovery already register for this vehicle on recovery no." + row['id']}
            return False
    ...
    if mydb.saveData(file_name, rows, ["id","vehicle_no","vehicle_type","recovery_on","recovery_status","handover_status","handover_on"]) :
        response_msg = {'status': 'success', 'msg': "recovery register in db on recovery no." + newRecordNo, 'item_inserted' : item}
        return True
...
def delete(vehicle_no, vehicle_type):
    ...
    if rows != rowsnew and mydb.saveData(file_name, rowsnew, ["id","vehicle_no","vehicle_type","recovery_on","recovery_status","handover_status","handover_on"]) :
...
def changeStatus(vehicle_no, vehicle_type, newstatus) :
    ...
    if not vehicle_no or not vehicle_type or not newstatus :
        response_msg = {'status': 'error', 'msg': "vehicle no, vehicle type and newstatus required."}
        return False
    
    rows = mydb.getData(file_name)
    for row in rows :
        if row['vehicle_no'] == vehicle_no and row['vehicle_type'] == vehicle_type :
            row['handover_status'] = newstatus
            row['handover_on'] = datetime.now()
            if mydb.saveData(file_name, rows, ["id","vehicle_no","vehicle_type","recovery_on","recovery_status","handover_status","handover_on"]) :
